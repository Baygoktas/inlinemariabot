...
@Client.on_message(~filters.channel & filters.text & ~filters.edited & filters.incoming)
...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("◀️", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"📃 Sayfa {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
    elif off_set is None:
        btn.append([InlineKeyboardButton(f"🗓 {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages"), InlineKeyboardButton("▶️", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            await query.answer('Beni bloklamışsın aq',show_alert = True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            return await query.answer("Önce kanala üye ol. Sinirlendirme beni.",show_alert=True)
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
            return await query.answer('Bulamadım bir şey.\nArama ipuçları için tıkla ve oku: /yardim')
        ...
        await query.message.edit_text(
            text=script.STATUS_TXT.format(total, users, chats, monsize, free) + stats,
            reply_markup=reply_markup,
            parse_mode='html'
        )
...
async def auto_filter(client, msg: Message, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"🗓 1/{round(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="▶️",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
